...
const MasterDataTable = ({ data, filters = {}, onFilterChange, statusOptions = [], collectionOptions = [], clientOptions = [] }) => {
            ...
            return (
                <div className="card-panel mt-8 p-6 text-center text-gray-400 border border-dashed border-gray-300">
                    ...
                    <button
                        ...
                        className="text-blue-600 hover:text-blue-800 text-sm font-bold mt-2 underline"
                    >
                        Limpiar Filtros
                    </button>
                </div>
    ...
    // Formatters
    const fmtMoney = (v) => v ? new Intl.NumberFormat('es-AR', { style: 'currency', currency: 'ARS' }).format(v) : '-';
            ...
            <div className="overflow-auto max-h-[500px]">
                <table className="w-full text-xs text-left whitespace-nowrap">
                    <thead className="bg-gray-50 text-gray-600 shadow-sm sticky top-0 z-20">
                        <tr>
                            {/* HEADER GROUP: COTIZACION */}
                            <HeaderCell label="N° Cot" field="id" widthClass="min-w-[80px]" stickyLeft isSticky
                                filters={filters} onFilterChange={handleFilterInput} sortConfig={sortConfig} handleSort={handleSort} />
                            <HeaderCell label="Fecha" field="date" widthClass="min-w-[90px]" isSticky
                                filters={filters} onFilterChange={handleFilterInput} sortConfig={sortConfig} handleSort={handleSort} />
                            <HeaderCell label="Cliente" field="client" widthClass="min-w-[150px]" isSticky
                                filters={filters} onFilterChange={handleFilterInput} sortConfig={sortConfig} handleSort={handleSort} clientOptions={optionsToUse} />
                            <HeaderCell label="Equipo" field="equipment" widthClass="min-w-[120px]" isSticky
                                filters={filters} onFilterChange={handleFilterInput} sortConfig={sortConfig} handleSort={handleSort} />
                            <HeaderCell label="Descripción" field="description" widthClass="max-w-[150px]" isSticky
                                filters={filters} onFilterChange={handleFilterInput} sortConfig={sortConfig} handleSort={handleSort} />
                            <HeaderCell label="Monto" field="amount" widthClass="min-w-[100px] text-right" isSticky
                                filters={filters} onFilterChange={handleFilterInput} sortConfig={sortConfig} handleSort={handleSort} />
                            <HeaderCell label="Estado" field="status" widthClass="min-w-[100px]" isSticky isDropdown options={statusOptions}
                                filters={filters} onFilterChange={handleFilterInput} sortConfig={sortConfig} handleSort={handleSort} />

                            {/* HEADER GROUP: VENTAS */}
                            <HeaderCell label="F. OC" field="ocDate" widthClass="bg-blue-50/30" isSticky
                                filters={filters} onFilterChange={handleFilterInput} sortConfig={sortConfig} handleSort={handleSort} />
                            <HeaderCell label="F. Factura" field="invoiceDate" widthClass="bg-blue-50/30" isSticky
                                filters={filters} onFilterChange={handleFilterInput} sortConfig={sortConfig} handleSort={handleSort} />
                            <HeaderCell label="F. Cobro" field="paymentDate" widthClass="bg-blue-50/30" isSticky
                                filters={filters} onFilterChange={handleFilterInput} sortConfig={sortConfig} handleSort={handleSort} />
                            <HeaderCell label="F. Entrega" field="deliveryDate" widthClass="bg-blue-50/30" isSticky
                                filters={filters} onFilterChange={handleFilterInput} sortConfig={sortConfig} handleSort={handleSort} />
                            <HeaderCell label="Costo" field="cost" widthClass="text-right bg-blue-50/30" isSticky
                                filters={filters} onFilterChange={handleFilterInput} sortConfig={sortConfig} handleSort={handleSort} />
                            <HeaderCell label="Ben %" field="profitPercent" widthClass="text-right bg-blue-50/30" isSticky
                                filters={filters} onFilterChange={handleFilterInput} sortConfig={sortConfig} handleSort={handleSort} />
                            <HeaderCell label="Ben $" field="profitAmount" widthClass="text-right bg-blue-50/30" isSticky
                                filters={filters} onFilterChange={handleFilterInput} sortConfig={sortConfig} handleSort={handleSort} />
                            <HeaderCell label="Cobrar Std" field="receivableStd" widthClass="text-right bg-blue-50/30" isSticky
                                filters={filters} onFilterChange={handleFilterInput} sortConfig={sortConfig} handleSort={handleSort} />
                            <HeaderCell label="Cobrar Real" field="receivableReal" widthClass="text-right bg-blue-50/30" isSticky
                                filters={filters} onFilterChange={handleFilterInput} sortConfig={sortConfig} handleSort={handleSort} />

                            <HeaderCell
                                label="Est. Cobro"
                                field="collectionStatus"
                                widthClass="bg-blue-50/30 min-w-[100px]"
                                isSticky
                                isDropdown
                            ...
                            <HeaderCell label="Desc Trabajo" field="finalDescription" widthClass="max-w-[150px] bg-blue-50/30" isSticky
                                filters={filters} onFilterChange={handleFilterInput} sortConfig={sortConfig} handleSort={handleSort} />
                            <HeaderCell label="Hs Cot" field="hoursQuoted" widthClass="text-center bg-blue-50/30" isSticky
                                filters={filters} onFilterChange={handleFilterInput} sortConfig={sortConfig} handleSort={handleSort} />
                            <HeaderCell label="Hs Uso" field="hoursUsed" widthClass="text-center bg-blue-50/30" isSticky
                                filters={filters} onFilterChange={handleFilterInput} sortConfig={sortConfig} handleSort={handleSort} />
                            <HeaderCell label="Póliza" field="policyIndex" widthClass="bg-blue-50/30" isSticky
                                filters={filters} onFilterChange={handleFilterInput} sortConfig={sortConfig} handleSort={handleSort} />
                            <HeaderCell label="Est. Póliza" field="policyStatus" widthClass="bg-blue-50/30" isSticky
                                filters={filters} onFilterChange={handleFilterInput} sortConfig={sortConfig} handleSort={handleSort} />
                            <HeaderCell label="OC N°" field="ocNumber" widthClass="bg-blue-50/30" isSticky
                                filters={filters} onFilterChange={handleFilterInput} sortConfig={sortConfig} handleSort={handleSort} />
                            <HeaderCell label="FC N°" field="invoiceNumber" widthClass="bg-blue-50/30" isSticky
                                filters={filters} onFilterChange={handleFilterInput} sortConfig={sortConfig} handleSort={handleSort} />
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-100">
                        {sortedData.map((row) => (
                            <tr key={row.id} className="hover:bg-blue-50/30 transition-colors">
                                ...
                                <td className="py-2.5 px-4 text-gray-500 max-w-xs truncate" title={row.description}>{fmtStr(row.description)}</td>
                                <td className="py-2.5 px-4 text-right font-mono text-gray-700">{row.amount ? fmtMoney(row.amount) : ''}</td>
                    ...
                    ${row.status === 'GANADA' ? 'bg-green-100 text-green-700'
                                ...
                                {row.isSold ? (
                                    <>
                                        ...
                                        <td className="py-2.5 px-4 bg-blue-50/5 text-right font-mono text-xs text-red-600">{fmtMoney(row.cost)}</td>
                                        <td className="py-2.5 px-4 bg-blue-50/5 text-right font-mono text-xs">{typeof row.profitPercent === 'number' ? (row.profitPercent * 100).toFixed(2) + '%' : '-'}</td>
                                        <td className="py-2.5 px-4 bg-blue-50/5 text-right font-mono font-medium text-green-600">{fmtMoney(row.profitAmount)}</td>
                                        <td className="py-2.5 px-4 bg-blue-50/5 text-right font-mono text-xs text-gray-600">{fmtMoney(row.receivableStd)}</td>
                                        <td className="py-2.5 px-4 bg-blue-50/5 text-right font-mono text-xs text-gray-600">{fmtMoney(row.receivableReal)}</td>
                                        <td className="py-2.5 px-4 bg-blue-50/5 text-gray-600">{fmtStr(row.collectionStatus)}</td>
                                        <td className="py-2.5 px-4 bg-blue-50/5 text-gray-500 max-w-xs truncate" title={row.finalDescription}>{fmtStr(row.finalDescription)}</td>
