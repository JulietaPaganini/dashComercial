            ...
            <div className="overflow-auto max-h-[600px]">
                <table className="w-full text-xs text-left whitespace-nowrap">
                    <thead className="bg-gray-50 text-gray-600 shadow-sm sticky top-0 z-20">
                        <tr>
                            <HeaderCell label="Cliente" field="client" widthClass="min-w-[200px]" stickyLeft isSticky
                                filters={filters} onFilterChange={handleFilterInput} sortConfig={sortConfig} handleSort={handleSort} clientOptions={optionsToUse} />
                            <HeaderCell label="Tipo" field="type" widthClass="min-w-[100px]" isSticky
                                filters={filters} onFilterChange={handleFilterInput} sortConfig={sortConfig} handleSort={handleSort} />
                            <HeaderCell label="N° Comp." field="number" widthClass="min-w-[100px]" isSticky
                                filters={filters} onFilterChange={handleFilterInput} sortConfig={sortConfig} handleSort={handleSort} />
                            <HeaderCell label="F. Emisión" field="date" widthClass="min-w-[90px]" isSticky
                                filters={filters} onFilterChange={handleFilterInput} sortConfig={sortConfig} handleSort={handleSort} />
                            <HeaderCell label="Vencimiento" field="dueDate" widthClass="min-w-[90px]" isSticky
                                filters={filters} onFilterChange={handleFilterInput} sortConfig={sortConfig} handleSort={handleSort} />
                            <HeaderCell label="Días Atraso" field="daysOverdue" widthClass="min-w-[80px] text-center" isSticky
                                filters={filters} onFilterChange={handleFilterInput} sortConfig={sortConfig} handleSort={handleSort} />
                            <HeaderCell label="Estado Antigüedad" field="agingBucket" widthClass="min-w-[100px]" isSticky
                                filters={filters} onFilterChange={handleFilterInput} sortConfig={sortConfig} handleSort={handleSort} />
                            <HeaderCell label="Importe" field="amount" widthClass="min-w-[100px] text-right" isSticky
                                filters={filters} onFilterChange={handleFilterInput} sortConfig={sortConfig} handleSort={handleSort} />
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-100">
                        {sortedData.map((row, idx) => (
                            <tr key={`${row.client}-${idx}`} className="hover:bg-blue-50/30 transition-colors">
                                <td className="py-2.5 px-4 font-medium text-gray-900 bg-white sticky left-0 z-10 border-r border-gray-100 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.05)]">
                                    {row.client}
                                </td>
                                <td className="py-2.5 px-4 text-gray-500">{row.type}</td>
                                <td className="py-2.5 px-4 text-gray-600 font-mono">{row.number || '-'}</td>
                                <td className="py-2.5 px-4 text-gray-600">{fmtDate(row.date)}</td>
                                <td className="py-2.5 px-4 text-gray-600">{fmtDate(row.dueDate)}</td>
                                <td className="py-2.5 px-4 text-center">
                                    <span className={`px-2 py-0.5 rounded font-bold ${row.daysOverdue > 90 ? 'bg-red-100 text-red-700' :
                                ...
                                <td className="py-2.5 px-4 text-gray-500">
                                    <span className="inline-block px-2 py-0.5 text-[10px] bg-gray-100 rounded-full border border-gray-200">
                                        {row.agingBucket}
                                    </span>
                                </td>
                                <td className="py-2.5 px-4 text-right font-mono font-medium text-gray-800">
                                    {fmtMoney(row.amount)}
                                </td>
                            </tr>
                        ))}
